...
test('public route: Client API 版本化契约支持 GET/HEAD 且不允许任意路径', async () => {
  const manifest = new FakeResponse();
  const manifestHandled = await handlePublicHttp(
    req('GET'),
    manifest as unknown as ServerResponse,
    new URL('http://local/contracts/client-api/v1/manifest.json'),
  );
  assert.equal(manifestHandled, true);
  assert.equal(manifest.statusCode, 200);
  assert.equal(manifest.headers['content-type'], 'application/json; charset=utf-8');
  assert.equal(JSON.parse(Buffer.from(manifest.body).toString('utf8')).contract, 'bailing.client-api');

  const schema = new FakeResponse();
  await handlePublicHttp(
    req('HEAD'),
    schema as unknown as ServerResponse,
    new URL('http://local/contracts/client-api/v1/run-request.schema.json'),
  );
  assert.equal(schema.statusCode, 200);
  assert.equal(schema.headers['content-type'], 'application/schema+json; charset=utf-8');
  assert.equal(schema.body.length, 0);

  const invalid = new FakeResponse();
  const invalidHandled = await handlePublicHttp(
    req('GET'),
    invalid as unknown as ServerResponse,
    new URL('http://local/contracts/client-api/v1/references/direct-protocol.json'),
  );
