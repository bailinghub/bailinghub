...
export class RouteRepository {
  ...
  private async write(r: Route, updateExisting: boolean): Promise<void> {
    await this.pool.query(
      'INSERT INTO bz_routes (route_key,name,enabled,target,target_config,project,profile,permission,session_policy,session_fixed_id,session_key_field,default_callback_url,delivery,knowledge,retry,tools,audience,memory,agent_client,budget,description,created_at,updated_at) ' +
        'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ' +
        (updateExisting
          ? 'ON DUPLICATE KEY UPDATE name=VALUES(name),enabled=VALUES(enabled),target=VALUES(target),target_config=VALUES(target_config),project=VALUES(project),profile=VALUES(profile),permission=VALUES(permission),session_policy=VALUES(session_policy),session_fixed_id=VALUES(session_fixed_id),session_key_field=VALUES(session_key_field),default_callback_url=VALUES(default_callback_url),delivery=VALUES(delivery),knowledge=VALUES(knowledge),retry=VALUES(retry),tools=VALUES(tools),audience=VALUES(audience),memory=VALUES(memory),agent_client=VALUES(agent_client),budget=VALUES(budget),description=VALUES(description),updated_at=VALUES(updated_at)'
          : ''),
      [r.route_key, r.name, r.enabled ? 1 : 0, r.target, r.target_config ? JSON.stringify(r.target_config) : null,
       ...
       r.agent_client && Object.keys(r.agent_client).length ? JSON.stringify(r.agent_client) : null,
