...
export class ToolProviderRepository {
  ...
  async updateSpecAccessProbe(name: string, probe: NonNullable<ToolProvider['spec_access_probe']>): Promise<void> {
    await this.pool.query('UPDATE bz_tool_providers SET spec_access_probe_json=?, updated_at=? WHERE name=?', [JSON.stringify(probe), dt(), name]);
  }
