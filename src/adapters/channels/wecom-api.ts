...
/**
 * One Kernel-owned WeCom credential cache. A multi-Kernel host must never keep
 * tenant secrets in a process singleton after that Kernel has been evicted.
 */
export class WecomAccessTokenCache {
  private readonly entries = new Map<string, { token: string; exp: number }>();
  private disposed = false;

  private key(corpid: string, secret: string): string {
    // Do not retain the plaintext secret as the Map key. The access token is
    // sensitive too and is cleared synchronously by dispose().
    return createHash('sha256').update(corpid).update('\0').update(secret).digest('hex');
  }

  async get(corpid: string, secret: string): Promise<string> {
    if (this.disposed) throw new Error('WeCom access token cache is disposed');
    const key = this.key(corpid, secret);
    ...
    if (this.disposed) throw new Error('WeCom access token cache is disposed');
    this.entries.set(key, { token: j.access_token, exp: Date.now() + (Number(j.expires_in ?? 7200) - 120) * 1000 }); // 提前 2min 过期
    return j.access_token;
  }

  invalidate(corpid: string, secret: string): void {
    this.entries.delete(this.key(corpid, secret));
  }
...
// Legacy direct imports retain their standalone behavior. Kernel composition
// always injects a dedicated cache and disposes it during Kernel.close().
const defaultAccessTokenCache = new WecomAccessTokenCache();
...
export async function sendWecomText(
  ...
  touser: string,
  content: string,
  accessTokenCache: WecomAccessTokenCache = defaultAccessTokenCache,
  ...
  let token = await accessTokenCache.get(corpid, secret);
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (j.errcode === 40014 || j.errcode === 42001) { // token 失效：刷新重试本条
      accessTokenCache.invalidate(corpid, secret);
      token = await accessTokenCache.get(corpid, secret);
...
export async function uploadWecomMedia(corpid: string, secret: string, type: 'image' | 'file' | 'voice' | 'video', buf: Buffer, filename: string, mime: string, accessTokenCache: WecomAccessTokenCache = defaultAccessTokenCache): Promise<{ ok: boolean; mediaId?: string; error?: string }> {
  ...
  try {
    ...
    if (j.errcode === 40014 || j.errcode === 42001) { accessTokenCache.invalidate(corpid, secret); token = await accessTokenCache.get(corpid, secret); j = await doUpload(token); }
...
export async function sendWecomMedia(corpid: string, secret: string, agentid: string | number, touser: string, msgtype: 'image' | 'file', mediaId: string, accessTokenCache: WecomAccessTokenCache = defaultAccessTokenCache): Promise<{ ok: boolean; errcode: number; errmsg: string }> {
...
export async function sendWecomCard(corpid: string, secret: string, agentid: string | number, touser: string, card: { title: string; description?: string; url: string; btntxt?: string }, accessTokenCache: WecomAccessTokenCache = defaultAccessTokenCache): Promise<{ ok: boolean; errcode: number; errmsg: string }> {
  const textcard = {
 ...
 */
export async function getWecomMedia(corpid: string, secret: string, mediaId: string, accessTokenCache: WecomAccessTokenCache = defaultAccessTokenCache): Promise<{ ok: boolean; buf?: Buffer; mime?: string; error?: string }> {
