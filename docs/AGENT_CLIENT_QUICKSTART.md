...
2. 只把允许本地 Agent 调用的写操作精确列入 `write_tools`；只读能力无需逐项加入该数组；
...
- `agent_authorize_url`：业务系统实现的 HTTPS 授权页面，例如
...
业务系统负责提供一个符合自身登录和权限体系的页面；页面必须先要求用户登录，再由业务后端从
当前服务端登录态推导 `principal`、`on_behalf_of` 和可授权 route。
