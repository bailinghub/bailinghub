...
- `max_calls`：全部业务工具源共享的本任务调用次数上限（防循环烧钱），默认 5，可由每条路由独立配置。每轮业务工具执行后，运行时会把已用和剩余次数作为可信状态交给模型；只剩一次时要求优先收敛，耗尽后移除工具并强制形成最终回答；
...
- 最终回答中的 DSML 等内部工具协议只会触发拦截与一次不带工具的安全重写，不会被展示或解析执行；连续违规失败关闭。流式客户端收到 `reset.reason=protocol_violation` 时必须清除本轮临时文本。
