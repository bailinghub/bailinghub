...
语义检索是白名单内的可选上下文增强，不是新的授权层。索引缓存采用 fresh / stale / miss 三态：过期快照继续服务并在后台单飞刷新，真正冷缓存只在部署级时限内等待；索引、凭证或查询向量阶段失败时回退到 catalog + `find_tools`。读取向量时必须复核模型、维度和字节长度，上游返回向量的实际长度也必须匹配请求维度；旧扩展缺少行级坐标时直接安全降级，不用另一次可变 snapshot 猜测坐标。同一 Provider 的重建串行执行并在锁内重读权威配置；embedding 模型或维度变化时必须原子替换整源索引，失败时保留旧坐标系。
