#!/usr/bin/env python3
"""Run a harmless DeepSeek-through-BailingHub bilingual E2E check.

The dedicated BailingHub client token is read from BAILINGHUB_TOKEN and is
never printed. The selected route must not expose business tools.
"""
...
DEFAULT_MARKER = "DEEPSEEK_BAILINGHUB_E2E_OK"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--route", required=True, help="A harmless, no-tool BailingHub route backed by DeepSeek")
    parser.add_argument(
        "--input",
        default=f"Return exactly {DEFAULT_MARKER}. Do not call any tool.",
        help="Harmless Chinese or English input sent through the selected route",
    )
    parser.add_argument("--marker", default=DEFAULT_MARKER, help="Expected marker in the authoritative job record")
...
def main() -> None:
    ...
    request_id = f"deepseek-e2e:{int(time.time())}:{uuid.uuid4().hex[:12]}"
    ...
    if status != "done":
        fail(f"job {job_id} ended with status={status or 'unknown'}")
    if not contains_marker(job, args.marker):
        fail(f"job {job_id} reached done but the expected marker was not found")

    print(f"PASS: bilingual DeepSeek-through-BailingHub E2E completed for job_id={job_id}")
