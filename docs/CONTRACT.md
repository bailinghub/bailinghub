...
**匿名预览边界**：控制台「聊天入口」里的独立预览链接始终不带业务票据，也不继承中枢管理员登录态；该页面没有可供业务用户再次登录的入口。无主体时，所有声明 `subject.required:true` 的工具（包括订单查询等只读能力）在装配阶段保持不可见。内置演示能力由管理员从「上手向导」运行 smoke 验证；真实网页助手则必须由业务后端在自己的登录态里签发下面的短票据。中枢管理员身份不得自动转换为业务操作主体。
