...
- **组件外观与运营启停**（「聊天入口」）：默认展开、窗口尺寸/高度、标题对齐、气泡贴左/右下角与像素偏移、头部头像、自定义气泡图标、底部品牌标识都可配置；入口可在列表一键暂停，已嵌入页面无需删除脚本，恢复后自动重新展示。
