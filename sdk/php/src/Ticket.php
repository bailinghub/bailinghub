...
/**
 * 签名访客票据（CONTRACT.md §1.1，聊天入口带登录身份）。
 *
...
final class Ticket
{
     ...
     */
    public static function sign(string $clientToken, string $uid, int $ttlSeconds = 7200, ?int $expiresAt = null): string
    {
        if ($uid === '' || strlen($uid) > 64) {
            throw new InvalidArgumentException('uid 长度需 1~64 字节');
        }
        $payload = rtrim(strtr(base64_encode(json_encode(
            ['uid' => $uid, 'exp' => $expiresAt ?? (time() + $ttlSeconds)],
